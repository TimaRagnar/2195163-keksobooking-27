...
const validateCapacity = () =>
  roomsToGuests[roomNumberSelect.value].includes(capacitySelect.value);

const getCapacityErrorMessage = () =>
  `Указанное количество комнта вмещает ${roomsToGuests[
    roomNumberSelect.value
  ].join(' или ')} комнаты.`;

const getRoomNumberErrorMessage = () =>
  `Для указанного количества гостей требуется ${guestsToRooms[
    capacitySelect.value
  ].join(' или ')} комнаты.`;

const getPriceErrorMessage = () =>
  `Цена выбранного типа жилья не менее ${
    minPrice[typeSelect.value]
  } рублей за ночь`;

const validatePrice = () =>
  Number(priceInput.value) >= Number(minPrice[typeSelect.value]);
